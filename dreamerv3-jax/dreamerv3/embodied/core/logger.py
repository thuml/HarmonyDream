  ...
  def _write(self, summaries):
    ...
    for step, name, value in summaries:
      if len(value.shape) == 0 and self._pattern.search(name):
        if step not in bystep.keys() or name not in bystep[step].keys():
          bystep[step][name] = []
        bystep[step][name].append(float(value))
    for step in bystep.keys():
      for name in bystep[step].keys():
        bystep[step][name]=np.mean(bystep[step][name])
    lines = ''.join([
...
class TensorBoardOutput(AsyncOutput):
  ...
  def _write(self, summaries):
    ...
    for step in bystep.keys():
      for name in bystep[step].keys():
         tf.summary.scalar(name, np.mean(bystep[step][name]), step)
    for step, name, value in summaries:
      try:
        if len(value.shape) == 0:
          # already handled
