...
class WorldModel(nj.Module):
  ...
  def train(self, data, state):
    ...
    if self.config.harmony:
      modules += [self.harmony_s1, self.harmony_s2, self.harmony_s3]
  ...
  def loss(self, data, state):
    ...
    if self.config.harmony:
      ...
      for key in losses.keys():
        if key == "reward":
          model_loss.append(self.harmony_s1(losses[key]))
        elif key == "image":
          model_loss.append(self.harmony_s2(losses[key]))
        elif key == "dyn":
          model_loss.append(self.harmony_s3(losses[key])*(self.scales["dyn"]/(self.scales["dyn"]+self.scales["rep"])))
        # only add the regularization term once
        elif key == "rep":
          model_loss.append(self.harmony_s3(losses[key], False)*(self.scales["rep"]/(self.scales["dyn"]+self.scales["rep"])))
        else:
          model_loss.append(losses[key])
      model_loss = sum(model_loss)
    else:
      scaled = {k: self.identity(v) * self.scales[k] for k, v in losses.items()}
