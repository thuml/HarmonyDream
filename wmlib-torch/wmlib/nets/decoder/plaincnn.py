...
class PlainCNNDecoder(BaseDecoder):
    ...
    def _cnn(self, features):
        ...
        if self._shapes["image"] == (160, 256, 3):
            x = self.get("convin", nn.Linear, features.shape[-1], 384 * 8 * 14)(features)
            x = torch.reshape(x, [-1, 384, 8, 14]).to(memory_format=torch.channels_last)
            output_paddings = [0, 0, 1, 0]
        else:
        ...
        for i, kernel in enumerate(self._cnn_kernels):
            ...
            x = self.get(f"conv{i}", ConvT, x.shape[1], depth, kernel, 2, output_padding=output_paddings[i])(x)
